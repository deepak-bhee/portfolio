...
const LightRays = ({
  ...
  useEffect(() => {
    ...
    // Use threshold:0 so fixed/sticky elements (which may report 0% intersection) still trigger
  ...
  // Initialise WebGL when component becomes visible
  useEffect(() => {
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // Gentle top-to-bottom brightness fade, equal across all channels
  float brightness = mix(0.5, 1.0, 1.0 - (coord.y / iResolution.y));
...
void main() {
  ...
  useEffect(() => {
    ...
    const onTouch = e => {
      if (!containerRef.current || !e.touches.length) return;
